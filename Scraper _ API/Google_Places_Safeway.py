#The code below extracts the Points of Interest located around Safeway Stores using Google Places API.
...
#This is the CSV file which acts as a source of input by providing the latitudes and longitudes of the Safeway Stores around which Points of #Interest have to be found.
csv_File = open('Safeway/Safeway_Locations.csv', 'r')
csv_read = csv.reader(csv_File)

url_base = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='

csv_Output = open('POI_near_Safeway.csv', 'w')
		...
		for k in Store_Type.keys():
			for typ in Store_Type[k]:
				...
				for place in data:
					...
					csvWriter.writerow(['Safeway', lati, loni, plc_nm, plc_string, plc_lat, plc_lon, plc_icon, plc_id, plc_vicnity])
